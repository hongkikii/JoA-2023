package com.mjuAppSW.joA.common.data;

//@Component
//@RequiredArgsConstructor
//@Profile("prod")
//public class InitProdMysql {
//
//    private final InitService initService;
//
//    @PostConstruct
//    public void init() {
//        initService.initCollege();
//        initService.initVoteCategory();
//        initService.initReportCategory();
//    }
//
//    @Component
//    @Transactional
//    @RequiredArgsConstructor
//    static class InitService {
//
//        private final EntityManager em;
//
//        MCollege c1;
//        VoteCategory vc1, vc2, vc3, vc4, vc5, vc6, vc7, vc8, vc9;
//        ReportCategory rc1, rc2, rc3;
//
//        public void initCollege() {
//            c1 = new MCollege(1L, "명지대학교", "@mju.ac.kr");
//            em.persist(c1);
//        }
//
//        public void initVoteCategory() {
//            vc1 = new VoteCategory("선배님 밥 사주세요!");
//            vc2 = new VoteCategory("혹시 3대 500?");
//            vc3 = new VoteCategory("이 강의실의 패피는 바로 너");
//            vc4 = new VoteCategory("페이커 뺨 칠 거 같음");
//            vc5 = new VoteCategory("친해지고 싶어요");
//            vc6 = new VoteCategory("과탑일 거 같아요");
//            vc7 = new VoteCategory("팀플 같이 하고 싶어요");
//            vc8 = new VoteCategory("끝나고 뭐 하는지 궁금해요");
//            vc9 = new VoteCategory("존잘/존예이십미다");
//
//            em.persist(vc1);
//            em.persist(vc2);
//            em.persist(vc3);
//            em.persist(vc4);
//            em.persist(vc5);
//            em.persist(vc6);
//            em.persist(vc7);
//            em.persist(vc8);
//            em.persist(vc9);
//        }
//
//        public void initReportCategory() {
//            rc1 = new ReportCategory("욕설/비방");
//            rc2 = new ReportCategory("성희롱");
//            rc3 = new ReportCategory("기타");
//
//            em.persist(rc1);
//            em.persist(rc2);
//            em.persist(rc3);
//        }
//    }
//}
